...
public class PlayerController : MonoBehaviour {
	...
	void Update () {
        if(SceneManager.GetActiveScene().name == "StopTrain"){
            Destroy(FindObjectOfType<DialogueController>().gameObject);
        ...
        if (stopPlayer == false)
        {
                    ...
                    if (inMenu == false)
                    {
                        ...
                        if (inConversation == false)
                        {
                            ...
                            if (moveHorizontal > 0.0f)
                            {
                                ...
                                if(squeakyWheel == true && squeak.isPlaying == false)
                                {
                                    squeak.Play();
                                }
                            }
            ...
            RaycastHit hitMisc;
            if (Physics.Raycast(transform.position, transform.TransformDirection(Vector3.forward), out hitMisc, rayMaxDistance, layerMask1))
            {
                if (hitMisc.transform.tag == "DogBowl")
                {
                    if (hitMisc.transform.gameObject.GetComponent<DogBowl>().hasFood == false)
                    {
                        if (inMenu == true && highlightedPos == 0)
                        {
                            if (Input.GetKeyDown(KeyCode.Space))
                            {
                                for (int i = 0; i < inventorySlot.Length; i++)
                                {
                                    if (inventorySlot[i] == 7)
                                    {
                                        inventorySlot[i] = 0;
                                        hitMisc.transform.gameObject.GetComponent<DogBowl>().hasFood = true;
                                        break;
                                    }
                                }
                            }
    ...
    private void OnTriggerStay(Collider other)
    {
        ...
        else if (other.tag == "Chef")
        {
            ...
            lookingAtSpeaker = true;
            if (FindObjectOfType<Chef>().frozen == false)
            {
                e.SetActive(true);
            }
        }
        ...
        {
            inConversation = other.transform.parent.GetComponent<Toilet>().inConversation;
            lookingAtSpeaker = true;
            e.SetActive(true);
        }
        else if (other.tag == "Hobo")
        {
            inConversation = other.transform.parent.GetComponent<HoboRobot>().inConversation;
            lookingAtSpeaker = true;
            e.SetActive(true);
        }
        else if (other.tag == "Nozzle")
        {
            inConversation = other.transform.parent.GetComponent<Nozzle>().inConversation;
            lookingAtSpeaker = true;
            e.SetActive(true);
        }
        else if (other.tag == "Guard")
        {
            inConversation = other.transform.parent.GetComponent<Guard>().inConversation;
            lookingAtSpeaker = true;
            if(other.transform.parent.GetComponent<Guard>().playerWin == true)
            {
                canMove = false;
                StartCoroutine(FadeOutCoRoutine("StopTrain"));
            }
            e.SetActive(true);
        }
        else
    ...
    private void WhatDialogueClass(RaycastHit hit)
    {
        ...
        {
            inConversation = hit.collider.gameObject.GetComponent<BathroomRobot>().inConversation;
        }
    ...
    private IEnumerator FadeOutCoRoutine(string whichScene)
    {
        ...
        else if (whichScene == "StopTrain")
        {
            SceneManager.LoadScene(7);
        }
        else if (whichScene == "Credits")
        {
